...
export default function GlobalReportFilters({ value, onChange, options = { employees: [], clients: [] }, show = {}, statusChoices = statusOptions, typeChoices = typeOptions }) {
  const set = (key) => (event) => onChange({ ...value, [key]: event.target.value });
  const roles = [...new Set(options.employees.map((item) => item.role).filter(Boolean))].sort();
  const setQuickPeriod = (kind) => {
    const today = new Date();
    const start = new Date(today.getFullYear(), today.getMonth(), kind === 'month' ? 1 : today.getDate() - (today.getDay() || 7) + 1);
    const end = kind === 'month' ? new Date(today.getFullYear(), today.getMonth() + 1, 0) : new Date(start.getFullYear(), start.getMonth(), start.getDate() + 6);
    const localKey = (date) => new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 10);
    onChange({ ...value, dateFrom: localKey(start), dateTo: localKey(end) });
  };
  return (
    <section className="surface-card p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-1"><button type="button" onClick={() => setQuickPeriod('week')} className="quiet-button min-h-9 px-3 py-1.5">Semana</button><button type="button" onClick={() => setQuickPeriod('month')} className="quiet-button min-h-9 px-3 py-1.5">Mês</button><button type="button" onClick={() => onChange(initialReportFilters)} className="quiet-button min-h-9 px-3 py-1.5"><RotateCcw className="h-4 w-4" />Limpar</button></div>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
        ...
        {show.status && <Field label="Status"><select className="input-base min-h-11" value={value.status} onChange={set('status')}><option value="">Todos</option>{statusChoices.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></Field>}
        {show.type && <Field label="Tipo"><select className="input-base min-h-11" value={value.type} onChange={set('type')}><option value="">Todos</option>{typeChoices.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></Field>}
